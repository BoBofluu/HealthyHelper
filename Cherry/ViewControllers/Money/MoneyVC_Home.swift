...
class MoneyVC_Home: BaseVC {
    ...
    override func viewDidLoad() {
        ...
        // 設定類型 - 支出
        bindTypePickerAction(type: .cost, picker: costPickerVC, UnSelectRow: indexCost, aryType: ["吃飯", "交通", "衣服"])
        // 設定類型 - 收入
        ...
        lDate.text = Date().stringValue(dTimeFormatDate)
        // 設定金額計算機
        bindMoneyAction()
        // 設定保存按鈕
...
//                clearValue()
    ...
    private func calculatorSpecial(_ special: String, isChange: String = "") {
        if checkEmpty() && isMath { // 不為空 且 前面為數字時
            lMoney.text = lMoney.text! + special
            if isChange != "" {
    ...
    @IBAction func doClickDate(_ sender: Any) {
        datePickerVC.m_confirmAction = {
            ...
            self.lDate.text = self.datePickerVC.selectedDate.stringValue(dTimeFormatDate)
        }
